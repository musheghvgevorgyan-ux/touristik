 ...
 * and Content API (hotel-content-api/1.0).
 *
...
class HotelbedsAdapter implements SupplierInterface
{
    ...
    {
        $this->apiKey    = config('suppliers.hotelbeds.api_key', '');
        $this->apiSecret = config('suppliers.hotelbeds.api_secret', '');

        $isLive = config('suppliers.hotelbeds.environment', 'test') === 'live';
    ...
    public function search(array $params): array
    {
        ...
        // -- Cache lookup --
        $cacheKey = 'hotelbeds_search_' . md5(
            $destination . $latitude . $longitude . $checkIn . $checkOut
            . $adults . $children . implode(',', $childAges)
        );
        ...
        // -- Write cache (1 hour) --
        if (!empty($hotels)) {
            Cache::put($cacheKey, $hotels, now()->addHour());
        }
    ...
    // ───────────────────────────────────────────────────────────
    //  Private -- HTTP transport (Laravel Http facade)
    // ───────────────────────────────────────────────────────────
    ...
    private function makeRequest(string $method, string $endpoint, ?array $body = null): array
    {
        ...
        $signature = hash('sha256', $this->apiKey . $this->apiSecret . time());
    ...
    private function enrichWithContentImages(array $hotels): array
    {
        ...
        try {
            ...
            if (!$response->successful()) {
                return $hotels; // graceful fallback
            }

            $data = $response->json();
        } catch (\Throwable $e) {
            Log::warning('Hotelbeds Content API image enrichment failed', [
                'error' => $e->getMessage(),
            ]);
