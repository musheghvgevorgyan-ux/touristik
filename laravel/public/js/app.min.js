...
(function () {
    ...
    // Mobile: Tours dropdown toggles submenu
    if (dropdownLink && submenu) {
        var touchedDropdown = false;
        dropdownLink.addEventListener('touchstart', function (e) {
            if (window.innerWidth <= 768) {
                touchedDropdown = true;
            }
        }, { passive: true });
        dropdownLink.addEventListener('touchend', function (e) {
            if (window.innerWidth <= 768 && touchedDropdown) {
                e.preventDefault();
                submenu.classList.toggle('open');
                touchedDropdown = false;
            }
        }, { passive: false });
        dropdownLink.addEventListener('click', function (e) {
            if (window.innerWidth <= 768) {
                e.preventDefault();
                // Only toggle if not already handled by touch
                if (!touchedDropdown) {
                    // Non-touch click (shouldn't happen on mobile but just in case)
                    submenu.classList.toggle('open');
                }
                touchedDropdown = false;
            }
    ...
    // Close menu when any nav link is clicked (including submenu links)
