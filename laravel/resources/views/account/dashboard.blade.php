...
@section('title', 'Dashboard - Touristik')
...
<div class="dashboard-page">
    <div class="dashboard-welcome reveal">
        <h1 data-t="dashboard_welcome">Welcome back, {{ $user['first_name'] ?? 'Traveler' }}!</h1>
        ...
        @if(!empty($recentBookings))
            <div class="bookings-table-wrap">
                <table class="bookings-table">
                    ...
                    <tbody>
                        @foreach(collect($recentBookings)->take(5) as $booking)
                            <tr>
                                <td><a href="/account/bookings/{{ $booking['id'] ?? '' }}" style="color:var(--primary);text-decoration:none;font-weight:600;">{{ $booking['reference'] ?? '---' }}</a></td>
                                <td>{{ ucfirst($booking['product_type'] ?? 'hotel') }}</td>
                                <td>{{ $booking['name'] ?? $booking['hotel_name'] ?? '---' }}</td>
                                <td>{!! !empty($booking['check_in']) ? \Carbon\Carbon::parse($booking['check_in'])->format('M d') : '---' !!}{!! !empty($booking['check_out']) ? ' - ' . \Carbon\Carbon::parse($booking['check_out'])->format('M d') : '' !!}</td>
                                <td>
                                    @php
                                ...
                                <td>{!! isset($booking['total_price']) ? ($booking['currency'] ?? 'USD') . " " . number_format($booking['total_price'], 2) : '---' !!}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
