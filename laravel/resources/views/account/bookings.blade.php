...
@section('title', 'My Bookings - Touristik')
            ...
            @foreach($bookings as $booking)
                ...
                <div class="booking-card reveal">
                    ...
                    <div class="booking-details">
                        <h3>
                            <a href="/account/bookings/{{ $booking['id'] ?? '' }}">
                                {{ $booking['name'] ?? $booking['hotel_name'] ?? 'Booking' }}
                            </a>
                        </h3>
                        <div class="booking-meta">
                            <span data-t="booking_ref">Ref: {{ $booking['reference'] ?? '---' }}</span>
                            <span>{{ ucfirst($type) }}</span>
                            @if(!empty($booking['check_in']))
                                <span>{{ \Carbon\Carbon::parse($booking['check_in'])->format('M d, Y') }}{!! !empty($booking['check_out']) ? ' - ' . \Carbon\Carbon::parse($booking['check_out'])->format('M d, Y') : '' !!}</span>
                            @endif
                        </div>
                    </div>
                    <div class="booking-right">
                        ...
                        @if(isset($booking['total_price']))
                            <span class="price">{!! ($booking['currency'] ?? 'USD') . " " . number_format($booking['total_price'], 2) !!}</span>
                        @endif
                    </div>
                </div>
            @endforeach
