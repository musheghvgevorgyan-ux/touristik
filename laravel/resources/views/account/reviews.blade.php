...
@section('title', 'My Reviews - Touristik')
            ...
            @foreach($reviews as $review)
                ...
                <div class="review-card reveal">
                    ...
                    <div class="review-meta">
                        @if(!empty($review['booking_reference']))
                            <span>Booking: {{ $review['booking_reference'] }}</span>
                        @endif
                        @if(!empty($review['product_type']))
                            <span>{{ ucfirst($review['product_type']) }}</span>
                        @endif
                        <span>{{ \Carbon\Carbon::parse($review['created_at'] ?? '')->format('M j, Y') }}</span>
                        <span class="review-status {!! $statusClass !!}">{{ ucfirst($status) }}</span>
                    </div>
            ...
            <form method="POST" action="/account/reviews" id="reviewForm">
                ...
                <div class="form-group">
                    ...
                    <select id="booking_id" name="booking_id" required>
                        ...
                        @foreach($pendingBookings as $b)
                            <option value="{!! (int) $b['id'] !!}" {!! old('booking_id') == $b['id'] ? 'selected' : '' !!}>
                                {{ $b['reference'] }} &mdash; {{ ucfirst($b['product_type'] ?? 'Booking') }} ({{ \Carbon\Carbon::parse($b['created_at'] ?? '')->format('M j, Y') }})
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="star-rating-selected" id="ratingText">
                        @php
                            $oldRating = (int) old('rating', '0');
                            if ($oldRating > 0) {
                                $labels = [1 => 'Poor', 2 => 'Fair', 3 => 'Good', 4 => 'Very Good', 5 => 'Excellent'];
                                echo e($labels[$oldRating] ?? '');
                            }
                        @endphp
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea id="review_comment" name="comment" rows="5"
                              placeholder="Tell us about your experience..." required minlength="10" maxlength="2000">{{ old('comment', '') }}</textarea>
