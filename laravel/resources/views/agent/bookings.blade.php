...
@section('title', 'Agent Bookings - Touristik')
    ...
    <form method="GET" action="/agent/bookings" class="agent-filter-bar">
        <div class="filter-grid">
            <div class="filter-field">
                ...
                <input type="text" id="search" name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Reference or guest name">
            </div>
            <div class="filter-field">
        ...
        <table class="agent-table">
            ...
            <tbody>
                ...
                @else
                    ...
                    @endphp
                    <tr>
                        <td><strong><a href="/agent/bookings/{!! (int)$b['id'] !!}" style="color:#2c5364;text-decoration:none;">{{ $b['reference'] }}</a></strong></td>
                        <td>{{ $b['guest_first_name'] . ' ' . $b['guest_last_name'] }}</td>
                        <td>{{ ucfirst($b['product_type'] ?? 'hotel') }}</td>
                        <td>{!! $checkIn ? \Carbon\Carbon::parse($checkIn)->format('M d, Y') : '-' !!}</td>
                        <td>{{ $b['currency'] ?? 'USD' }} {!! number_format((float)($b['net_price'] ?? 0), 2) !!}</td>
                        <td>{{ $b['currency'] ?? 'USD' }} {!! number_format((float)($b['sell_price'] ?? 0), 2) !!}</td>
                        <td class="commission-col">{{ $b['currency'] ?? 'USD' }} {!! number_format((float)($b['commission'] ?? 0), 2) !!}</td>
                        <td><span class="badge badge-{{ $b['status'] ?? 'pending' }}">{{ ucfirst($b['status'] ?? 'pending') }}</span></td>
                        <td><span class="badge badge-{{ $b['payment_status'] ?? 'unpaid' }}">{{ ucfirst($b['payment_status'] ?? 'unpaid') }}</span></td>
                        <td>{!! \Carbon\Carbon::parse($b['created_at'] ?? date('Y-m-d')->format('M d, Y')) !!}</td>
                    </tr>
                    @endforeach
                @endif
            </tbody>
