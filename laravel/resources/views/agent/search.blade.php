...
@section('title', 'Hotel Search - Agent Portal')
    ...
    <form method="POST" action="/agent/search" class="agent-search-form">
        ...
        <div class="search-grid">
            <div class="search-field">
                ...
                <input type="text" id="destination" name="destination" value="{{ $destination }}" placeholder="City name (e.g. Dubai, Antalya)" required>
            </div>
            <div class="search-field">
                <label for="check_in">Check-in</label>
                <input type="date" id="check_in" name="check_in" value="{{ $checkIn }}" min="{!! date('Y-m-d') !!}" required>
            </div>
            <div class="search-field">
                <label for="check_out">Check-out</label>
                <input type="date" id="check_out" name="check_out" value="{{ $checkOut }}" min="{!! date('Y-m-d', strtotime('+1 day')) !!}" required>
            </div>
            <div class="search-field">
                ...
                <div class="agent-hotel-body">
                    ...
                    <div class="agent-hotel-pricing">
                        <div class="price-net">
                            ...
                            <div class="price-amount">{{ $currency }} {!! number_format($netPrice, 2) !!}</div>
                            <div class="price-per-night">{{ $currency }} {!! number_format(round($netPrice / $nights, 2), 2) !!} / night</div>
                        </div>
                        <div class="price-sell">
                            <div class="price-label">Suggested Sell Price</div>
                            <div class="price-amount">{{ $currency }} {!! number_format($suggestedSell, 2) !!}</div>
                            <div class="commission-info">+{{ $currency }} {!! number_format($commission, 2) !!} commission</div>
                        </div>
                        <div class="agent-hotel-actions">
                            <form method="POST" action="/booking/create/hotel/0?rate_key={!! urlencode($hotel['rate_key'] ?? '') !!}&rate_type={{ $hotel['rate_type'] ?? 'BOOKABLE' }}" style="display:inline">
                                @csrf
