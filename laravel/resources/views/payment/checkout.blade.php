...
@section('title', 'Payment - Touristik')
            ...
            <div class="checkout-summary">
                <div class="checkout-card">
                    ...
                    <div class="checkout-ref">
                        ...
                        <span class="checkout-ref-code">{{ $booking['reference'] ?? '' }}</span>
                    </div>
                    ...
                    @if($hotelName)
                        <div class="checkout-detail">
                            <h4>{{ $hotelName }}</h4>
                            @if(!empty($product['destination']))
                                <p class="checkout-dest">&#128205; {{ $product['destination'] }}</p>
                            @endif
                        </div>
                    @endif
                    ...
                    <div class="checkout-info-grid">
                        ...
                        <div class="checkout-info-item">
                            ...
                            <span class="checkout-value">{{ \Carbon\Carbon::parse($checkIn)->format('D, M d Y') }}</span>
                        </div>
                        @endif
                        @if($checkOut)
                        <div class="checkout-info-item">
                            <span class="checkout-label">Check-out</span>
                            <span class="checkout-value">{{ \Carbon\Carbon::parse($checkOut)->format('D, M d Y') }}</span>
                        </div>
                        @endif
                        <div class="checkout-info-item">
                            <span class="checkout-label">Guest</span>
                            <span class="checkout-value">{{ ($booking['guest_first_name'] ?? '') . ' ' . ($booking['guest_last_name'] ?? '') }}</span>
                        </div>
                        <div class="checkout-info-item">
                            <span class="checkout-label">Status</span>
                            <span class="checkout-value">{{ ucfirst($booking['status'] ?? 'confirmed') }}</span>
                        </div>
                        @if(!empty($product['rooms']))
                        <div class="checkout-info-item">
                            <span class="checkout-label">Rooms</span>
                            <span class="checkout-value">{!! count($product['rooms']) !!}</span>
                        </div>
                        @endif
                        @if(!empty($product['board']) || !empty($product['board_name']))
                        <div class="checkout-info-item">
                            <span class="checkout-label">Board</span>
                            <span class="checkout-value">{{ $product['board_name'] ?? $product['board'] ?? '' }}</span>
                        </div>
                        @endif
                    </div>
                    ...
                    <div class="checkout-total">
                        ...
                        <span class="checkout-total-price">{!! ($currency) . " " . number_format($totalPrice, 2) !!}</span>
                    </div>
                    ...
                    @if(!empty($booking['payments']) && count($booking['payments']) > 0)
                        <div class="checkout-payments-history">
                            ...
                            @foreach($booking['payments'] as $p)
                                <div class="checkout-payment-row checkout-payment-{{ $p['status'] ?? 'pending' }}">
                                    <span>{{ ucfirst($p['gateway'] ?? '') }} - {{ ucfirst($p['status'] ?? '') }}</span>
                                    <span>{!! "$" . number_format((float, 2)($p['amount'] ?? 0), $p['currency'] ?? $currency) !!}</span>
                                </div>
                            @endforeach
                        </div>
                    @endif
                    ...
                    <div class="checkout-option-body">
                        ...
                        <form method="POST" action="/payment/process" class="checkout-form" id="sandboxPaymentForm">
                            ...
                            <button type="submit" class="btn checkout-btn checkout-btn-pay">
                                &#128274; Pay {!! ($currency) . " " . number_format($totalPrice, 2) !!}
                            </button>
                        </form>
                    </div>
                </div>
