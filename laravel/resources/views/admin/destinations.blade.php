...
@section('title', 'Manage Destinations - Touristik')
    ...
    <form method="POST" action="/admin/destinations" id="destinationForm">
        ...
        <div class="form-grid">
            <div class="form-group">
                ...
                <input type="text" name="name" id="dest-name" required placeholder="e.g. Paris" value="{{ old('name') }}">
            </div>
            <div class="form-group">
                <label for="dest-slug">Slug</label>
                <input type="text" name="slug" id="dest-slug" required placeholder="e.g. paris" value="{{ old('slug') }}">
            </div>
            <div class="form-group full-width">
                <label for="dest-description">Description</label>
                <textarea name="description" id="dest-description" placeholder="Brief description of the destination...">{{ old('description') }}</textarea>
            </div>
            <div class="form-group">
                <label for="dest-country">Country</label>
                <input type="text" name="country" id="dest-country" placeholder="e.g. France" value="{{ old('country') }}">
            </div>
            <div class="form-group">
                <label for="dest-price">Price From</label>
                <input type="number" name="price_from" id="dest-price" step="0.01" min="0" placeholder="e.g. 299.00" value="{{ old('price_from') }}">
            </div>
            <div class="form-group full-width">
                <label for="dest-image">Image URL</label>
                <input type="url" name="image_url" id="dest-image" placeholder="https://..." value="{{ old('image_url') }}">
            </div>
            <div class="form-group">
                <label for="dest-color">Card Color</label>
                <input type="color" name="color" id="dest-color" value="{{ old('color', '#FF6B35') }}">
            </div>
            <div class="form-group">
                <label for="dest-emoji">Emoji</label>
                <input type="text" name="emoji" id="dest-emoji" placeholder="e.g. &#127468;&#127463;" maxlength="4" value="{{ old('emoji') }}">
            </div>
            <div class="form-group">
                ...
                <tr>
                    ...
                    <td>
                        @if(!empty($dest['image_url']))
                            <img src="{{ $dest['image_url'] }}" alt="{{ $dest['name'] }}" class="dest-thumb" loading="lazy">
                        @else
                            <span class="dest-thumb" style="display:inline-block;background:#eee;text-align:center;line-height:35px;font-size:1.2rem;">{{ $dest['emoji'] ?? '?' }}</span>
                        @endif
                    </td>
                    <td><strong>{{ $dest['name'] }}</strong></td>
                    <td><code>{{ $dest['slug'] }}</code></td>
                    <td>{{ $dest['country'] ?? '-' }}</td>
                    <td>{!! isset($dest['price_from']) ? '$' . number_format($dest['price_from'], 2) : '-' !!}</td>
                    <td>
                        @if(!empty($dest['featured']))
                            <span class="dest-featured-yes">Yes</span>
                        @else
                            <span class="dest-featured-no">No</span>
                        @endif
                    </td>
                    <td><span class="badge badge-{{ $dest['status'] ?? 'active' }}">{{ ucfirst($dest['status'] ?? 'Active') }}</span></td>
                    <td>
                        <div class="action-btns">
                            <button type="button" class="btn-edit" onclick="editDestination({{ json_encode($dest) }})">Edit</button>
