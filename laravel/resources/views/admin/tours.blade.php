    ...
    <form method="POST" action="/admin/tours" id="tourForm">
        ...
        <div class="form-grid">
            <div class="form-group">
                ...
                <input type="text" name="title" id="tour-title" required placeholder="e.g. Classic Yerevan Tour" value="{{ old('title') }}">
            </div>
            <div class="form-group">
                <label for="tour-slug">Slug</label>
                <input type="text" name="slug" id="tour-slug" placeholder="e.g. classic-yerevan-tour" value="{{ old('slug') }}">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="text" name="duration" id="tour-duration" placeholder="e.g. 3 days" value="{{ old('duration') }}">
            </div>
            <div class="form-group full-width">
                <label for="tour-description">Description</label>
                <textarea name="description" id="tour-description" placeholder="Describe the tour...">{{ old('description') }}</textarea>
            </div>
            <div class="form-group">
                <label for="tour-price">Price From</label>
                <input type="number" name="price_from" id="tour-price" step="0.01" min="0" placeholder="e.g. 199.00" value="{{ old('price_from') }}">
            </div>
            <div class="form-group">
                ...
                <select name="destination_id" id="tour-destination">
                    ...
                    @foreach($destinations as $dest)
                        <option value="{!! (int)$dest['id'] !!}">{{ $dest['name'] }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group full-width">
                <label for="tour-image">Image URL</label>
                <input type="url" name="image_url" id="tour-image" placeholder="https://..." value="{{ old('image_url') }}">
            </div>
            <div class="form-group">
                ...
                <tr>
                    ...
                    <td>
                        @if(!empty($tour['image_url']))
                            <img src="{{ $tour['image_url'] }}" alt="{{ $tour['title'] }}" class="tour-thumb" loading="lazy">
                        @else
                    ...
                    <td>
                        @if(!empty($tour['featured']))
                            <span class="tour-featured-yes">Yes</span>
                        @else
                            <span class="tour-featured-no">No</span>
                        @endif
                    </td>
                    <td><span class="badge badge-{{ $tour['status'] ?? 'active' }}">{{ ucfirst($tour['status'] ?? 'Active') }}</span></td>
                    <td>
                        <div class="action-btns">
                            <button type="button" class="btn-edit" onclick='editTour({{ json_encode($tour) }})'>Edit</button>
