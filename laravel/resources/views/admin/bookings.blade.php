...
@section('title', 'Manage Bookings - Touristik')
...
<form method="GET" action="/admin/bookings" class="admin-filter-bar">
    ...
    <div class="admin-filter-group">
        ...
        <input type="text" name="search" id="filter-search" placeholder="Reference or guest name..." value="{{ $filters['search'] ?? '' }}">
    </div>
    <div class="admin-filter-group">
            ...
            @else
                @foreach($bookings as $booking)
                    ...
                    <tr>
                        ...
                        <td>{{ ucfirst($booking['product_type'] ?? 'N/A') }}</td>
                        <td>{{ $productData['hotel_name'] ?? $productData['tour_name'] ?? '-' }}</td>
                        <td>{!! !empty($booking['check_in']) ? \Carbon\Carbon::parse($booking['check_in'])->format('M d, Y') : '-' !!}</td>
                        <td><span class="badge badge-{{ $booking['status'] }}">{{ ucfirst($booking['status']) }}</span></td>
                        <td><span class="badge badge-{{ $booking['payment_status'] ?? 'pending' }}">{{ ucfirst($booking['payment_status'] ?? 'Pending') }}</span></td>
                        <td><strong>{{ $booking['currency'] ?? 'USD' }} {!! number_format($booking['sell_price'] ?? 0, 2) !!}</strong></td>
                        <td>{{ \Carbon\Carbon::parse($booking['created_at'])->format('M d, Y') }}</td>
                        <td><a href="/admin/bookings/{{ $booking['id'] }}" class="view-link">View &rarr;</a></td>
                    </tr>
                @endforeach
            @endif
        </tbody>
