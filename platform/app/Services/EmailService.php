 ...
 *
 * Migrated from the legacy functions.php emailTemplate() / sendHtmlEmail().
...
class EmailService
{
    ...
    public static function template(string $title, string $body, string $ctaHtml = ''): string
    {
        ...
        // The entire email is built as a single string with inline styles.
        // Using table-based layout for Outlook / older clients.
        return '<!DOCTYPE html>'
            ...
            .   'background:#203a43;'  // fallback for clients that ignore gradients
    ...
    public static function send(string $to, string $subject, string $htmlBody, string $replyTo = ''): bool
    {
        ...
        $replyAddr = $replyTo ?: $config['reply_to'];
    ...
    public static function sendBookingConfirmation(string $guestEmail, string $guestName, array $bookingData): bool
    {
        ...
        $voucherUrl = 'https://touristik.am/booking/' . urlencode($bookingData['reference'] ?? '');
    ...
    /**
     * Load mail config with sensible defaults.
     */
    private static function config(): array
    {
        ...
        return array_merge([
            'from_name'   => 'Touristik Travel',
            'from_email'  => 'noreply@touristik.am',
            'reply_to'    => 'info@touristik.am',
            'admin_email' => 'info@touristik.am',
        ], $config);
    }
    ...
    {
        // Try the settings table first (matches legacy getSetting($pdo, 'contact_email'))
        try {
        ...
        $config = self::config();
        return $config['admin_email'] ?? 'info@touristik.am';
    }
}
