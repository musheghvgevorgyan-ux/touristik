        ...
        <div class="form-grid">
            <div class="form-group">
                ...
                <div class="code-input-wrap">
                    <input type="text" name="code" id="promo-code" required placeholder="SUMMER2026" value="<?= View::old('code') ?>">
            ...
            <div class="form-group">
                ...
                <input type="number" name="value" id="promo-value" step="0.01" min="0" required placeholder="e.g. 10" value="<?= View::old('value') ?>">
            </div>
            <div class="form-group">
                <label for="promo-min-order">Minimum Order ($)</label>
                <input type="number" name="min_order" id="promo-min-order" step="0.01" min="0" placeholder="e.g. 100" value="<?= View::old('min_order') ?>">
            </div>
            <div class="form-group">
                <label for="promo-max-discount">Max Discount ($)</label>
                <input type="number" name="max_discount" id="promo-max-discount" step="0.01" min="0" placeholder="e.g. 50 (percentage type only)" value="<?= View::old('max_discount') ?>">
            </div>
            <div class="form-group">
                <label for="promo-usage-limit">Total Usage Limit</label>
                <input type="number" name="usage_limit" id="promo-usage-limit" min="0" placeholder="0 = unlimited" value="<?= View::old('usage_limit') ?>">
            </div>
            <div class="form-group">
                <label for="promo-per-user">Per User Limit</label>
                <input type="number" name="per_user_limit" id="promo-per-user" min="0" placeholder="0 = unlimited" value="<?= View::old('per_user_limit') ?>">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="datetime-local" name="starts_at" id="promo-starts" value="<?= View::old('starts_at') ?>">
            </div>
            <div class="form-group">
                <label for="promo-expires">Expires At</label>
                <input type="datetime-local" name="expires_at" id="promo-expires" value="<?= View::old('expires_at') ?>">
            </div>
        ...
        <tbody>
            ...
            <?php else: ?>
                <?php foreach ($promos as $promo): ?>
                    ...
                    ?>
                <tr>
                    <td><span class="promo-code"><?= View::e($promo['code']) ?></span></td>
                    <td><?= View::e(ucfirst($promo['type'] ?? 'percentage')) ?></td>
                    <td>
                        <?php if (($promo['type'] ?? 'percentage') === 'percentage'): ?>
                    ...
                    <td>
                        <?php if (!empty($promo['starts_at']) || !empty($promo['expires_at'])): ?>
                            <?= !empty($promo['starts_at']) ? View::date($promo['starts_at'], 'M d, Y') : 'N/A' ?>
                            &ndash;
                            <?= !empty($promo['expires_at']) ? View::date($promo['expires_at'], 'M d, Y') : 'N/A' ?>
                        <?php else: ?>
                            No limit
                        <?php endif; ?>
                    </td>
                    <td><span class="badge badge-<?= View::e($statusClass) ?>"><?= View::e($statusLabel) ?></span></td>
                    <td>
                        <div class="action-btns">
                            <button type="button" class="btn-edit" onclick="editPromo(<?= View::e(json_encode($promo)) ?>)">Edit</button>
