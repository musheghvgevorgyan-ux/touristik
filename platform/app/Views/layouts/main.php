...
$siteName = SettingsService::get('site_name', 'Touristik Travel Club');
$gaId = SettingsService::get('ga_measurement_id', '');
$footerText = SettingsService::get('footer_text', '© 2026 Touristik. All rights reserved.');
$rates = CurrencyService::getRates();
$currentUrl = View::currentUrl();
?>
<!DOCTYPE html>
<html lang="<?= View::e($user['language'] ?? 'en') ?>">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title><?= View::e($title ?? 'Touristik') ?></title>
    ...
    <meta property="og:title" content="<?= View::e($title ?? 'Touristik') ?>">
    ...
    <meta property="og:url" content="https://touristik.am<?= View::e($currentUrl) ?>">
...
<body>
    ...
    <header>
        <nav aria-label="Main navigation">
            <div class="logo"><a href="/"><img src="/img/logo-transparent.png" alt="<?= View::e($siteName) ?>" class="logo-img"></a></div>
            <button class="hamburger" id="hamburger" aria-label="Menu">
                <span></span><span></span><span></span>
            </button>
            <ul class="nav-links">
                <li><a href="/" <?= $currentUrl === '/' ? 'class="active"' : '' ?> data-t="home">Home</a></li>
                <li class="nav-dropdown">
                    <a href="/tours" <?= str_starts_with($currentUrl, '/tours') ? 'class="active"' : '' ?> data-t="tours_nav">Tours <span class="nav-arrow">&#9662;</span></a>
                    <ul class="nav-submenu">
                        <li><a href="/tours/ingoing" <?= $currentUrl === '/tours/ingoing' ? 'class="active"' : '' ?> data-t="tour_cat_ingoing">Ingoing Tours</a></li>
                        <li><a href="/tours/outgoing" <?= $currentUrl === '/tours/outgoing' ? 'class="active"' : '' ?> data-t="tour_cat_outgoing">Outgoing Tours</a></li>
                        <li><a href="/tours/transfer" <?= $currentUrl === '/tours/transfer' ? 'class="active"' : '' ?> data-t="tour_cat_transfer">Transfer</a></li>
                    </ul>
                </li>
                <li><a href="/destinations" <?= str_starts_with($currentUrl, '/destinations') ? 'class="active"' : '' ?> data-t="destinations">Destinations</a></li>
                <li><a href="/about" <?= $currentUrl === '/about' ? 'class="active"' : '' ?> data-t="about">About</a></li>
                <li><a href="/contact" <?= $currentUrl === '/contact' ? 'class="active"' : '' ?> data-t="contact">Contact</a></li>
                <?php if ($user): ?>
                    <li><?php include BASE_PATH . '/app/Views/partials/notifications-dropdown.php'; ?></li>
                    <li><a href="/account" <?= str_starts_with($currentUrl, '/account') ? 'class="active"' : '' ?> data-t="my_account">My Account</a></li>
                    <?php if (in_array($user['role'], ['admin', 'superadmin'])): ?>
                        <li><a href="/admin" <?= str_starts_with($currentUrl, '/admin') ? 'class="active"' : '' ?> data-t="admin">Admin</a></li>
                    <?php endif; ?>
                    <?php if ($user['role'] === 'agent'): ?>
                        <li><a href="/agent" <?= str_starts_with($currentUrl, '/agent') ? 'class="active"' : '' ?>>Agent Portal</a></li>
                    <?php endif; ?>
                    <li><a href="/logout" data-t="logout">Logout</a></li>
                <?php else: ?>
                    <li><a href="/login" <?= $currentUrl === '/login' ? 'class="active"' : '' ?> data-t="login">Login</a></li>
                <?php endif; ?>
            </ul>
            <div class="nav-controls">
                ...
                <div class="currency-switcher">
                    ...
                    <div class="lang-dropdown" id="currencyDropdown">
                        ...
                        <a href="#" class="lang-option currency-opt" data-symbol="&euro;" data-code="EUR" data-rate="<?= $rates['EUR'] ?>">&euro; EUR</a>
    ...
    <main id="main-content" role="main">
        <?php if (!empty($flash)): ?>
            <?php foreach ($flash as $type => $message): ?>
                <?php if (is_string($message) && in_array($type, ['success', 'error', 'warning', 'info'])): ?>
                    <div class="flash flash-<?= View::e($type) ?>" style="max-width:1200px;margin:1rem auto;padding:0.8rem 1.2rem;border-radius:8px;
            ...
            <?php if (isset($flash['errors']) && is_array($flash['errors'])): ?>
                <div class="flash flash-error" style="max-width:1200px;margin:1rem auto;padding:0.8rem 1.2rem;border-radius:8px;background:#f8d7da;color:#721c24;border:1px solid #f5c6cb;">
    ...
    <footer>
        ...
        <div class="footer-bottom">
            <p data-t="footer_text"><?= View::e($footerText) ?></p>
        </div>
    </footer>
