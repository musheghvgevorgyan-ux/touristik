        ...
        <div class="notif-list" id="notifList">
            <?php if (!empty($notifRecent)): ?>
                <?php foreach ($notifRecent as $n): ?>
                    ...
                    ?>
                    <a href="<?= View::e($n['link'] ?? '#') ?>"
                       class="notif-item <?= $isRead ? '' : 'unread' ?>"
                       data-notif-id="<?= (int) $n['id'] ?>"
                       data-read="<?= $isRead ? '1' : '0' ?>">
                        <div class="notif-icon <?= View::e($nType) ?>"><?= $nIcon ?></div>
                        <div class="notif-content">
                            <div class="notif-title"><?= View::e($n['title'] ?? '') ?></div>
                            <div class="notif-msg"><?= View::e($n['message'] ?? '') ?></div>
                            <div class="notif-time"><?= View::e($timeAgo) ?></div>
                        </div>
                        <?php if (!$isRead): ?>
